...
export async function generateGroups(req, res) {
  ...
  try {
    ...
    // 1ï¸âƒ£ Run the service â€“ creates the groups & saves them
    const groups = await createGroupsForCurriculum(curriculum, groupSize, teacherId);

    // 2ï¸âƒ£ Populate each group with the teacher name (if any)
    const populated = await Promise.all(
      groups.map((g) => ClassGroup.findById(g._id).populate("teacher", "fullName email employeeRole employmentStatus"))
    );
 ...
 *       Queryâ€‘param: curriculum=Mathematics%20101
 */
